  ...
  onSubmit() {
    ...
    if (this.userForm.valid) {
      this.fromSubmitted.emit({
        ...
        update: this.editUser !== null,
      });
    }
  }

  getAnimationTrigger(): string {
    return this.isRightButtonClick
      ? "slideFadeAnimation"
      : "slideFadeAnimationLeft";
  }
    ...
    switch (this.selectedSection) {
      case ClientFormSectionNames.PERSONAL:
        return (
          ...
          this.userForm.get("personalId")?.invalid
        );
      case ClientFormSectionNames.LEGAL_ADDRESS:
        return (
          this.userForm.get("legalAddress.country")?.invalid ||
          this.userForm.get("legalAddress.city")?.invalid ||
          this.userForm.get("legalAddress.address")?.invalid ||
