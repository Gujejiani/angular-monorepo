      ...
      return apiCall.pipe(
        ...
        map((users: any) => {
          const alreadyUsed = users?.some(
            (user: UserModel) => user.personalId === control.value
          );
...
export function patternValidator(
  ...
  return (control: AbstractControl): { [key: string]: string } | null => {
    ...
    if (
      (georgianPattern.test(value) || englishPattern.test(value)) &&
      !containsNumbers
    ) {
      return null;
    } else {
      return {
        errorMessage: "Value must only contain Georgian or Latin letters",
      };
    }
  };
}
