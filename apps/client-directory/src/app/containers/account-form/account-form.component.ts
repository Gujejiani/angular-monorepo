...
export class AccountFormComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    const user = structuredClone(
      this.users?.find(
        (user) => String(this.formGroup.value.clientId) === String(user.id)
      )
    );
  ...
  setForm() {
    ...
    this.formGroup = this.formBuilder.group({
      currency: [CurrencyEnum.GEL, Validators.required],
      accountType: [AccountTypeEnum.CURRENT, Validators.required],
